...
e.on("list users", str => {
    callSafely(cli.responders.listUsers)(str);
});
...
e.on("more order info", str => {
    callSafely(cli.responders.moreOrderInfo)(str);
});
...
cli.responders.listUsers = async str => {
    ...
    const arr = str.split('--');
    // If the user typed --all, then pick all users, otherwise just the ones from the last 24 hours
    const timeThreshold = typeof (arr[1]) == "string" && ["all"].indexOf(arr[1].trim()) > -1 ? now : 24 * 60 * 60 * 1000;
    ...
    await Promise.all(userIds.map(async userId => {
        ...
        if (now - userData.createdAt <= timeThreshold) {
            count++;
...
cli.responders.moreUserInfo = async str => {
    ...
    if (userId) {
        const userData = await _data.read("users", userId).catch(_ => {
            // Handle invalid id specifically to avoid printing the entire error from Node to the user
...
cli.responders.listOrders = async str => {
    ...
    await Promise.all(orders.map(async orderId => {
        ...
        if (now - orderData.orderPlacedAt <= timeThreshold) {
            count++;
            // Print order info
            console.log(`ID: ${orderData.id}\tPlaced by: ${orderData.recipient}\tTotal amount: ${orderData.totalAmount}`);
            cli.verticalSpace();
        }
    }));
...
// More order info
cli.responders.moreOrderInfo = async str => {
    // Get the order id
    const arr = str.split('--');
    const orderId = typeof (arr[1]) == "string" && arr[1].trim().length > 0 ? arr[1].trim() : false;

    if (orderId) {
        const orderData = await _data.read("orders", orderId).catch(_ => {
            // Handle invalid id specifically to avoid printing the entire error from Node to the user
